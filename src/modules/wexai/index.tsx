...
interface AIResponse {
  sohbet: string;
  kisiSonuclari: Record<string, unknown>[];
  gsmSonucu?: { GSM: string } | undefined;
}
...
async function callAIService(message: string, history: { role: string, content: string }[]): Promise<AIResponse> {
  ...
  return { 
    sohbet: data.sohbet, 
    kisiSonuclari: data.kisiSonuclari || [], 
    gsmSonucu: data.gsmSonucu || undefined 
  };
}
...
export default function WexAIPage() {
  ...
  // Loading bekleme mesajı için state
